import { Github, Check, AlertCircle } from "lucide-react";
import { signIn } from "@/auth";
import { prisma } from "@/lib/db";
import { requireUserId } from "@/app/actions/_helpers";
import {
  disconnectGoogle,
  disconnectGitHub,
} from "@/app/actions/connections";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";

interface PageProps {
  searchParams: { error?: string };
}

export default async function SettingsPage({ searchParams }: PageProps) {
  const userId = await requireUserId();
  const accounts = await prisma.account.findMany({
    where: { userId },
    select: { provider: true },
  });

  const google = accounts.find((a) => a.provider === "google");
  const github = accounts.find((a) => a.provider === "github");
  const onlyOne = accounts.length <= 1;

  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      <header className="pt-2">
        <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
          settings · connected accounts
        </p>
        <h1 className="mt-1 text-3xl font-semibold tracking-tight">
          Connections
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Link Google for calendar access and GitHub for pull-request data.
          Different emails are fine — both attach to your single user.
        </p>
      </header>

      {searchParams.error && (
        <div className="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <AlertCircle className="h-4 w-4" />
          {searchParams.error}
        </div>
      )}

      <ProviderCard
        title="Google"
        description="Identity and calendar"
        icon={<GoogleLogo />}
        connected={Boolean(google)}
        connectAction={async () => {
          "use server";
          await signIn("google", { redirectTo: "/settings" });
        }}
        disconnectAction={disconnectGoogle}
        canDisconnect={!onlyOne}
      />

      <ProviderCard
        title="GitHub"
        description="Pull request data"
        icon={<Github className="h-5 w-5" />}
        connected={Boolean(github)}
        connectAction={async () => {
          "use server";
          await signIn("github", { redirectTo: "/settings" });
        }}
        disconnectAction={disconnectGitHub}
        canDisconnect={!onlyOne}
      />
    </div>
  );
}

function ProviderCard({
  title,
  description,
  icon,
  connected,
  connectAction,
  disconnectAction,
  canDisconnect,
}: {
  title: string;
  description: string;
  icon: React.ReactNode;
  connected: boolean;
  connectAction: () => Promise<void>;
  disconnectAction: () => Promise<void>;
  canDisconnect: boolean;
}) {
  return (
    <Card>
      <CardHeader className="flex-row items-center gap-4 space-y-0">
        <div className="flex h-10 w-10 items-center justify-center rounded-md border bg-background">
          {icon}
        </div>
        <div className="flex-1">
          <CardTitle className="text-base">{title}</CardTitle>
          <CardDescription className="text-xs">{description}</CardDescription>
        </div>
        {connected ? (
          <span className="flex items-center gap-1.5 rounded-full bg-secondary px-2.5 py-0.5 text-[11px] font-medium text-secondary-foreground">
            <Check className="h-3 w-3" />
            connected
          </span>
        ) : (
          <span className="rounded-full border px-2.5 py-0.5 text-[11px] text-muted-foreground">
            not connected
          </span>
        )}
      </CardHeader>
      <CardContent className="flex justify-end pt-2">
        {connected ? (
          <form action={disconnectAction}>
            <Button
              type="submit"
              variant="ghost"
              size="sm"
              disabled={!canDisconnect}
              title={
                canDisconnect
                  ? undefined
                  : "Connect another provider before disconnecting this one"
              }
            >
              Disconnect
            </Button>
          </form>
        ) : (
          <form action={connectAction}>
            <Button type="submit" size="sm">
              Connect
            </Button>
          </form>
        )}
      </CardContent>
    </Card>
  );
}

function GoogleLogo() {
  return (
    <svg viewBox="0 0 18 18" className="h-4 w-4">
      <path
        d="M17.64 9.205c0-.638-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z"
        fill="#4285F4"
      />
      <path
        d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z"
        fill="#34A853"
      />
      <path
        d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"
        fill="#FBBC05"
      />
      <path
        d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z"
        fill="#EA4335"
      />
    </svg>
  );
}
